...
* to make this a lighter sized app, the data sets are truncated to 100 rows (or 100 in length for vectors) and a max of 30 columns
* the data listed under `base` really comes from the `datasets` package that is included on installation. You can find installed data from all packages using `data(package = .packages(all.available = TRUE))` (you can add `$results` to view it as a data frame)
