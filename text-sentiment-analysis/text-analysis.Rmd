...
```{r}
found_nounphrases <- spacyr::nounphrase_consolidate(spacy_sentences)
...
unique_text_nounphrases <-
  ...
  left_join(
    select(extract_sentences, ends_with("_id"), rating, date)
  ) |> 
  distinct(text_id, object_id, phrase, rating, date)


unique_text_nounphrases |> 
  mutate(split = rating > 3) |> 
...
as_sentiments |> 
  filter(rating >= 3) |>
  plot_word_rank_sentiment("rating", 6)

as_sentiments |>
  mutate(split = ifelse(rating <= 3, "3 & under", "4+")) |> 
  mutate(object_and_rating = (paste(split, object_id, sep = "\n"))) |> 
  plot_word_rank_sentiment("object_and_rating", word_n = 4, across_n = 4)
...
igraph::graph_from_data_frame(
  d = select(phrase_cor, word:correlation,  mean_edge = mean),
  vertices = distinct(phrase_cor, word, n, mean_point = mean)
) |>
